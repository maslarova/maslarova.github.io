...
kde $\mathbb{A}$ je regulárna matica typu $n \times n$, $\mathbf{b}$ je vektor pravej strany a $\mathbf{x}$ je riešenie sústavy. Ďalšou základnou úlohou je hľadanie vlastných čísel a vektorov matice.<br>
...
Jednotlivé prvky vektoru $x$ budeme počítať zdola hora, od $x_n$, $x_{n-1}$, až po $x_1$. 
